...
<div class="modal fixed inset-0 z-50 transition-opacity duration-300 flex items-center invisible opacity-0" {{ $attributes }}>

    <!-- BACKDROP -->
    <div class="modal-backdrop absolute inset-0 bg-stone-950/50 opacity-0 transition-opacity duration-100"></div>

    <!-- PANEL -->
    <div class="modal-panel mx-auto h-auto w-full transform scale-75 transition-transform duration-200 ease-in-out p-8 {{ $sizes[$size] ?? $sizes['small'] }}">
